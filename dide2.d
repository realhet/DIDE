...
{/// Workspace ///////////////////////////////////////////////
	...
	{//! Module handling ///////////////////////////////////////
		version(/+$DIDE_REGION+/all)
		{
			...
			@property
			{
				Module mainModule()
				{
					return findModule(mainModuleFile);
				} void mainModule(Module m)
				{
		...
		}version(/+$DIDE_REGION+/all)
		{
			...
			{//note: toJson: this can't be protected. But an array can (mixin() vs. __traits(member, ...).
				...
				{
					size_t res;
					foreach(idx, const layer; markerLayers) if(!layer.visible)
						res |= 1 << idx;
					return res;
				}
	...
	{
		auto paste_impl(bool dontMeasure=false)(TextSelection[] textSelections,
	...
	{//! Keyboard mapping ///////////////////////////////////////
		...
		}version(/+$DIDE_REGION+/all)
		{
			version(/+$DIDE_REGION More text selection+/all)
			{
				...
				{
					insertCursor(-1);
				} @VERB("Ctrl+Alt+Down") void insertCursorBelow()
				{
					insertCursor( 1);
				}
	...
	{//! UI /////////////////////////////////////////////////////////
		version(/+$DIDE_REGION+/all)
		{
			...
			({
				...
				if(searchBoxVisible)
				{
					...
					if(Btn(symbol("Zoom"), isFocused(editContainer) ? kcFindZoom : KeyCombo(""),
						enable(matchCnt>0), hint("Zoom screen on search results.")))
					{
						zoomAt(view, searchResults);
					}
		
					if(Btn("Sel", isFocused(editContainer) ? kcFindToSelection : KeyCombo(""),
		...
		}version(/+$DIDE_REGION+/all)
		{
			...
			{with(im){// UI_BuildMessageTypeBtn ///////////////////////////
			...
			{with(im){// UI_selectedModulesHint//////////////////////////
				auto sm = selectedModules;
				void stats()
				{
					Row(format!"(%d LOC, %sB)"(sm.map!(m => m.linesOfCode).sum,
				...
				({
					...
					{ 
						if(auto col = mod.content)
						{
							...
							{
								CodeRow[] visibleRows = col.rows.filter!(r => r.outerBounds.overlaps(visibleBounds)
									&& r.subCells.length).array;
								//opt: binary search
