...
import het, het.ui, het.tokenizer, het.keywords, het.stream, het.graph;
  ...
  override void onUpdate(){
    ...
    with(im) Panel(PanelPosition.topClient, {
      Row({
        ...
        if(actPage == Pages.dGrammar) dlangGrammarGraph.graph.UI_SearchBox(view);
        if(actPage == Pages.modules) moduleGraph.UI_SearchBox(view);
      });
    });


    if(actPage == Pages.dGrammar) dlangGrammarGraph.update(view);
    if(actPage == Pages.modules ) moduleGraph.update2(view);

    with(im) Panel(PanelPosition.topLeft, {
      width = 300;
      flags.vScrollState = ScrollState.auto_;

      if(actPage == Pages.dGrammar) dlangGrammarGraph.graph.UI_Editor;
      if(actPage == Pages.modules ) moduleGraph.UI_Editor;
    });
